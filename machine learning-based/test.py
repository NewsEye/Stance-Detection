...
Scorer for the Fake News Challenge
 - @bgalbraith
Submission is a CSV with the following fields: target, Body ID, Stance
...
FIELDNAMES = ['target', 'Body ID', 'Stance']
LABELS = ['POS', 'NEG', 'OBJ']
RELATED = LABELS[0:2]
...
def score_submission(gold_labels, test_labels):
    ...
    for i, (g, t) in enumerate(zip(gold_labels, test_labels)):
        if g['target'] != t['target'] or g['Body ID'] != t['Body ID']:
...
def print_confusion_matrix(cm):
    ...
    for i, row in enumerate(cm):
        hit += row[i]
        total += sum(row)
        lines.append("|{:^11}|{:^11}|{:^11}|{:^11}|".format(LABELS[i],*row))
        lines.append("-"*line_len)
