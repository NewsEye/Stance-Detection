...
def create_file_senti(dict_pred, file_name, num_context_word, lang):
...
#*** global stance detection ***
    senti_dict = {}
    if lang!='en' and os.path.exists(senti_file_path): 
        senti_dict = read_dict(senti_file_path)

     for num_context_word in range(3, 30, 2): # config context_word for validating, testing     
#    for num_context_word in range(19, 21, 2):               
        dict_pred = {}     
        for file_name in os.listdir(input_folder):
            if '.ipynb_checkpoints' in file_name: 
                continue
            dict_pred = create_file_senti(dict_pred, file_name, num_context_word, lang)
        
        write_dict(dict_pred, results_folder+'dict_pred.json')
        
        dict_gt = read_dict(results_folder+'dict_gt.json')    
        dict_pred = read_dict(results_folder+'dict_pred.json')
        y_true, y_pred=[],[]
        for key in dict_gt:
            if dict_gt[key]!='c': 
                y_true.append(dict_gt[key].replace('+','0').replace('-','1').replace('n','2'))
                y_pred.append(dict_pred[key].replace('+','0').replace('-','1').replace('n','2'))
        eval_result_dict = metrics.classification_report(y_true, y_pred, output_dict=True)
