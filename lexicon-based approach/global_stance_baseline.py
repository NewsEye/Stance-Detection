...
lang = 'EN'  # 'en'#'de';#'fr'
results_folder = data_folder + dataset + "_results_num/" 
...
#     list_tok_lemma = [tok.lemma_ for tok in nlp(src_txt)]
    ...
    if abs(avg_pos - avg_neg) <= pol_threshold: return 'n'
...
#     dict_NEs_senti = create_NEs_senti(file_name, dict_NEs, dict_toks, dict_NEs_senti, lang)
    ...
    list_context_word = create_NEs_senti(file_name, dict_NEs, dict_toks, dict_NEs_senti, num_context_word, lang)
...
# def create_csv_BERT(postprocess_data, dict_file_path, start_docid, end_docid):
# #     dict_file_path = "../json_data/polarity_data.json" 
#     dict_tmp = read_dict(dict_file_path)#"../json_data/polarity_data_ex.csv"
#     with open(postprocess_data, mode='w') as polar_data:
...
#         
#     results_folder=data_folder+dataset_name+"_results_num/"    
#     if not os.path.exists(results_folder):
#         os.mkdir(results_folder) 
# 
#     convert_to_right_format(postprocess_data,dict_file_path, start_docid, end_docid)
#     exit()
    
    #================ choose num_context_words, num_polar_score_step ================    
#     senti_dict = {}
#     if lang != 'en' and os.path.exists(senti_file_path): 
#         senti_dict = read_dict(senti_file_path)
# 
#     max_context_words=30;  max_polar_score_step=30
# 
# 
#     dict_gt = read_dict(results_folder + 'dict_gt.json')  
#     dict_pred = {}
#     for num_context_word in range(1, max_context_words, 1):
#             key_num_context_word = str(num_context_word)
#             if key_num_context_word not in dict_pred:
#                 dict_pred[key_num_context_word]={}
#                  
#             for file_name in os.listdir(input_folder):
#                 if '.ipynb_checkpoints' in file_name or '.txt' not in file_name: 
#                     continue
#                 print(file_name)
#                 list_context_word = create_file_senti(file_name, num_context_word,lang)
#                 
#                 for step in range(1, max_polar_score_step, 1):    
#                     pol_threshold = step *0.01 
#                     key_pol_threshold = str(pol_threshold) 
#                     if  key_pol_threshold not in dict_pred[key_num_context_word]:
#                         dict_pred[key_num_context_word][key_pol_threshold]={}
#                         
#                     ne_senti = ""
#                     if list_context_word != None and len(list_context_word) > 0: 
#                         ne_senti = get_senti_sent([list_context_word], pol_threshold, lang) 
#                     if ne_senti == "": dict_pred[key_num_context_word][key_pol_threshold][file_name] = "n"
#                     else: dict_pred[key_num_context_word][key_pol_threshold][file_name] = ne_senti
#     
# 
#     for num_context_word in range(1, max_context_words, 1):
#         key_num_context_word = str(num_context_word)
...
#             for key in dict_gt:
#                 if dict_gt[key] != 'c': 
#                     y_true.append(dict_gt[key].replace('+', '0').replace('-', '1').replace('n', '2'))
#                     y_pred.append(dict_pred[key_num_context_word][key_pol_threshold][key].replace('+', '0').replace('-', '1').replace('n', '2'))
#             eval_result_dict = metrics.classification_report(y_true, y_pred, output_dict=True)
#             write_dict(eval_result_dict, results_folder + "eval_result_dict_" +str(num_context_word) +"_"+ str(pol_threshold) + ".json")
#             print(results_folder)
#         
#     get_f1_all(results_folder, max_context_words,  max_polar_score_step) ;     exit()     
    ...
    dict_gt = read_dict(results_folder + 'dict_gt.json')  
    dict_pred = {}
    num_context_word=17; pol_threshold = 0.07
#     for num_context_word in range(1, max_context_words, 1):
    key_num_context_word = str(num_context_word)
    if key_num_context_word not in dict_pred:
        dict_pred[key_num_context_word]={}
